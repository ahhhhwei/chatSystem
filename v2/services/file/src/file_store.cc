...
FileStore::FileStore(std::filesystem::path storage_path)
    : storage_path_(std::move(storage_path)) {
    std::error_code error;
    std::filesystem::create_directories(storage_path_, error);
    if (error) {
...
std::optional<std::string> FileStore::put(
...
bool FileStore::get(
    ...
    std::ifstream input(
        storage_path_ / std::string(file_id),
        std::ios::binary);
    if (!input) {
...
const std::filesystem::path& FileStore::storage_path() const noexcept {
...
bool FileStore::is_valid_file_id(std::string_view file_id) noexcept {
    if (file_id.size() != 36) {
        return false;
    }
