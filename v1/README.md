...
ODB 在数据元结构定义时，使用预处理指令（#pragma）来提供元数据，这些元数据指示如何将C++类型映射到数据库模式

> #pragma：给编译器或工具传额外指示

以下是一些常用的 #pragma 指令：

| #   | ODB 指令                              | PDF 中的作用                                   | 直观理解                    |
| --- | ------------------------------------- | ---------------------------------------------- | --------------------------- |
| 1   | `#pragma db object`                   | 声明一个类是数据库对象，会映射到数据库中的表   | **类 → 表**                 |
| 2   | `#pragma db table("table_name")`      | 指定类映射到哪个数据库表；不指定则默认使用类名 | **指定表名**                |
| 3   | `#pragma db id`                       | 标记某个成员变量作为数据库表的主键             | **主键**                    |
| 4   | `#pragma db column("column_name")`    | 指定成员变量映射到数据库中的列名               | **指定列名**                |
| 5   | `#pragma db view`                     | 声明一个类是数据库视图，而不是普通表           | **数据库视图**              |
| 6   | `#pragma db session`                  | PDF 描述为用于声明数据库会话                   | **会话相关**                |
| 7   | `#pragma db query("query")`           | PDF 描述为用于定义自定义查询函数               | **自定义查询**              |
| 8   | `#pragma db index("index_name")`      | 指定成员变量应该被建立索引                     | **索引**                    |
| 9   | `#pragma db default("default_value")` | 指定成员变量对应列的默认值                     | **DEFAULT**                 |
| 10  | `#pragma db unique`                   | 指定成员变量或一组变量具有唯一性约束           | **UNIQUE**                  |
| 11  | `#pragma db not_null`                 | 指定成员变量对应字段不允许为空                 | **NOT NULL**                |
| 12  | `#pragma db auto`                     | 指定成员变量的值插入时自动生成，例如自增主键   | **AUTO_INCREMENT 类似概念** |
| 13  | `#pragma db transient`                | 指定成员变量不持久化到数据库                   | **这个变量不存数据库**      |
| 14  | `#pragma db type("type_name")`        | 指定成员变量对应的数据库类型                   | **指定 SQL 类型**           |
| 15  | `#pragma db convert("converter")`     | 指定自定义类型转换器                           | **C++ 类型 ↔ DB 类型转换**  |
| 16  | `#pragma db pool("pool_name")`        | 指定数据库连接池                               | **连接池相关**              |
| 17  | `#pragma db trigger("trigger_name")`  | 指定插入、更新或删除时触发的触发器             | **数据库 Trigger**          |
...
- nullable
  ...
  针对可能为空字段封装的类似智能指针的类型。由于 C++ 普通 `int` 没有 `NULL` 这种状态，所以 `nullable<int>` 可以表达有值或者 `NULL`
...
- `result<T>`
  ...
  odb 查询结果器，大致可以理解为 `std::vector<Student>` 
...
编写 [student.hxx](./tools-usage/odb/test2/student.hxx)，然后编译：
...
运行 [student.sql](./tools-usage/odb/test2/student.sql) 编写 [main.cc](./tools-usage/odb/test2/main.cc)，然后编译运行：

[](./pic/odb-test2.png)

### 4.11 RabbitMQ
RabbitMQ 是一个消息队列中间件，实现两个客户端主机之间消息传输的功能（发布&订阅），
> RabbitMQ 基于 AMQP 协议，AMQP 全称是 Advanced Message Queuing Protocol（高级消息队列协议）。

> 为什么要有消息队列？
> 假设没有消息队列，两个服务器之间如何发送消息？一种做法是基于 TCP，`Server A --> TCP --> Server B`，但是如果机器很多，服务器之间就会出现大量直接连接。RabbitMQ 的思想就是在中间放一个消息中间件，`Server A --> RabbitMQ --> Server B`
> 此外， RabbitMQ 加了一层 Exchange：`Producer --> Exchange --> Queue --> Consumer`，生产者不关心消息最终进入哪个 Queue。Exchange 是 RabbitMQ 内部的消息路由器。它收到消息正文和 `routing_key` 后，根据 `Exchange 类型 + Binding` 计算这个消息应该进入哪些 Queue
> 
### 4.12 DMS
